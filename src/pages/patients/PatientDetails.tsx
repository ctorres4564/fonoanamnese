...
export const PatientDetails: React.FC = () => {
  ...
  const getActionForStatus = (anamnesis: AnamnesisType) => {
    if (anamnesis.isArchived) {
      return { label: 'Visualizar', action: () => navigate(`/patients/${patient?.id}/anamneses/${anamnesis.id}/view`) };
    }
    switch (anamnesis.status) {
      case 'draft':
      case 'in_progress':
        return { label: 'Continuar', action: () => navigate(`/patients/${patient?.id}/anamneses/${anamnesis.id}/edit`) };
      case 'review':
        return { label: 'Revisar', action: () => navigate(`/patients/${patient?.id}/anamneses/${anamnesis.id}/review`) };
      default:
        return { label: 'Visualizar', action: () => navigate(`/patients/${patient?.id}/anamneses/${anamnesis.id}/view`) };
    }
  };
          ...
          <dl className="sm:divide-y sm:divide-gray-200">
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                {new Date(patient.birthDate).toLocaleDateString('pt-BR', { timeZone: 'UTC' })} ({calculateAge(patient.birthDate)} anos)
              </dd>
            </div>
          </dl>
        </div>
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-8">
        <div className="px-4 py-5 sm:px-6 flex justify-between items-center bg-gray-50">
          ...
          <div className="flex gap-4 items-center">
            <Link to={`/patients/${patient.id}/anamneses`} className="text-sm text-indigo-600 hover:underline">
            ...
            <ul className="divide-y divide-gray-200">
              {recentAnamneses.map(anamnesis => {
                ...
                const updatedDate = anamnesis.updatedAt 
                  ? new Date((anamnesis.updatedAt as any).seconds ? (anamnesis.updatedAt as any).seconds * 1000 : anamnesis.updatedAt).toLocaleDateString('pt-BR')
                  : 'N/A';
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        ...
                        <div className="mt-2 flex items-center text-sm text-gray-500 gap-4">
                          <span>
                            Atualizada em {updatedDate}
                          </span>
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                            {translateStatus(anamnesis.status)} {anamnesis.isArchived ? '(Arquivada)' : ''}
                          </span>
