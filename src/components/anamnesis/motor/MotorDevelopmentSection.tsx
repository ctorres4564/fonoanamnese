...
import type { MotorDevelopmentSection as MotorSectionType } from '../../../types/anamnesis';
import { motorDevelopmentSchema } from '../../../schemas/motorDevelopment';

function MotorDevelopmentFields() {
  const { register, watch, formState: { errors } } = useFormContext<{ motorDevelopment: MotorSectionType }>();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm mt-6 space-y-6">
        <h3 className="text-lg font-medium text-gray-800 mb-4 border-b pb-2">7. Condições Gerais</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Desenvolvimento percebido como compatível com a idade</label>
            <select 
        ...
        {reportedMotorDelay === 'sim' && (
          <div>
            <label className="block text-sm font-medium text-gray-700">Descrição do atraso motor *</label>
            <input 
              type="text" 
            ...
            {getError('motorDevelopment.general.motorDelayDescription') && (
              <p className="mt-1 text-sm text-red-600">{getError('motorDevelopment.general.motorDelayDescription') as string}</p>
            )}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Dificuldade de equilíbrio</label>
            <select 
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Dificuldade para correr</label>
            <select 
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Dificuldade para subir/descer escadas</label>
            <select 
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Dificuldade com objetos pequenos</label>
            <select 
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Dificuldade com ferramentas (lápis, talheres)</label>
            <select 
          ...
          <div className="space-y-4">
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4">
              <p className="text-sm text-yellow-700">
                <strong>Aviso:</strong> Quedas recorrentes devem ser analisadas considerando idade, ambiente, desenvolvimento e condições de saúde.
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">Descrição (Frequência, contexto, consequências)</label>
              <textarea 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Idade em que foi percebida</label>
            <input 
              type="text" 
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Uso de órtese, prótese ou dispositivo de mobilidade</label>
          <select 
        ...
        {usesMobilityDevice === 'sim' && (
          <div>
            ...
            {getError('motorDevelopment.general.mobilityDeviceType') && (
              <p className="mt-1 text-sm text-red-600">{getError('motorDevelopment.general.mobilityDeviceType') as string}</p>
            )}
          </div>
        )}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Descrição de movimentos repetitivos</label>
          <input 
            type="text" 
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Observações adicionais (Geral)</label>
          <textarea 
