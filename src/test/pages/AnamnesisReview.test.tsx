  ...
  return render(
    <MemoryRouter initialEntries={['/patients/patient-1/anamneses/anamnesis-1/review']}>
      <Routes>
        ...
        <Route path="/patients/:patientId/anamneses/:anamnesisId/view" element={<div data-testid="view-page" />} />
        <Route path="/patients/:patientId/anamneses/:anamnesisId/edit" element={<div data-testid="edit-page" />} />
      </Routes>
    </MemoryRouter>
  );
};
...
describe('AnamnesisReview', () => {
  ...
  it('deve listar as seções com seus status (Completa, Incompleta, Não iniciada)', async () => {
    ...
    (validation.validateSection as any).mockImplementation((sectionId: string) => {
      ...
      if (sectionId === 'chiefComplaint') return { isValid: false, errors: ['Falta algo'], filledFieldsCount: 2 };
  ...
  it('deve desabilitar finalização e mostrar aviso se CREFONO estiver ausente', async () => {
    (authContext.useAuth as any).mockReturnValue({ user: mockUser, profile: { crefonoNumber: '' } });
