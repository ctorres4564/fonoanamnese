...
export const createGuardian = async (
  guardianData: Omit<Guardian, 'id' | 'createdAt' | 'updatedAt'>,
): Promise<string> => {
...
export const updateGuardian = async (
  guardianId: string,
  guardianData: Partial<
    Omit<Guardian, 'id' | 'professionalId' | 'patientId' | 'createdAt' | 'updatedAt'>
  >,
): Promise<void> => {
  if (guardianData.isPrimaryContact) {
    ...
    // Busca todos
    const q = query(collection(db, GUARDIANS_COLLECTION), where('patientId', '==', patientId))
    const querySnapshot = await getDocs(q)
...
export const listGuardiansByPatient = async (
