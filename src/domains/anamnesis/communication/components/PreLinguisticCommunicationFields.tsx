...
export function PreLinguisticCommunicationFields() {
  const { register } = useFormContext<{ communicationDevelopment: CommunicationDevelopmentSection }>();

  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-6">
      <h3 className="text-lg font-medium text-gray-800 border-b pb-2">1. Comunicação Pré-Linguística</h3>
      
      <div className="space-y-6">
        {behaviors.map(({ key, label }) => (
          <div key={key} className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start border-b border-gray-100 pb-4 last:border-0">
            <div>
              <p className="font-medium text-sm text-gray-700 mb-2">{label}</p>
              <select
                {...register(`communicationDevelopment.preLinguisticCommunication.${key}.status` as any)}
            ...
            <div>
              ...
              <textarea
                {...register(`communicationDevelopment.preLinguisticCommunication.${key}.observation` as any)}
                rows={2}
                className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
