    ...
    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-3">
          Como a criança se comunica principalmente? (Selecione todos que se aplicam)
        </label>
      ...
      {hasOther && (
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray-700">
            Descrição para outro modo de comunicação *
          </label>
          <input
            type="text"
          ...
          {getError('communicationDevelopment.communicationModeOtherDescription') && (
            <p className="mt-1 text-sm text-red-600">
              {getError('communicationDevelopment.communicationModeOtherDescription') as string}
            </p>
          )}
        </div>
      )}
