  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm space-y-6">
      <h3 className="text-lg font-medium text-gray-800 border-b pb-2">
        6. Regressão Comunicativa Geral
      </h3>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700">
            Houve perda de habilidades comunicativas?
          </label>
          <select
        ...
        <div className="bg-red-50 border border-red-200 p-4 rounded-md space-y-4">
          <div className="flex items-start">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-red-700">
                <p>
                  Foi relatada perda de habilidade comunicativa previamente adquirida. Essa
                  informação requer investigação profissional e não constitui diagnóstico.
                </p>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              {getError('communicationDevelopment.communicationRegression.lostSkill') && (
                <p className="mt-1 text-sm text-red-600">
                  {getError('communicationDevelopment.communicationRegression.lostSkill') as string}
                </p>
              )}
            </div>
            <div>
              ...
              <input
                type="number"
                min="0"
                {...register('communicationDevelopment.communicationRegression.regressionAge', {
                  valueAsNumber: true,
                })}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              />
              {getError('communicationDevelopment.communicationRegression.regressionAge') && (
                <p className="mt-1 text-sm text-red-600">
                  {
                    getError(
                      'communicationDevelopment.communicationRegression.regressionAge',
                    ) as string
                  }
                </p>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              >
                <option value="">Selecione...</option>
                {REGRESSION_ONSET_MODES.map((opt) => (
                  <option key={opt} value={opt}>
                    {opt}
                  </option>
                ))}
            ...
            {onsetMode === 'outro' && (
              <div>
                ...
                <input
                  type="text"
                  {...register(
                    'communicationDevelopment.communicationRegression.onsetModeOtherDescription',
                  )}
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                />
                {getError(
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">
                Foi realizada avaliação profissional?
              </label>
              <select
                {...register(
                  'communicationDevelopment.communicationRegression.professionalEvaluation',
                )}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              >
                <option value="">Selecione...</option>
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700">
          Observações gerais sobre regressão
        </label>
        <textarea
