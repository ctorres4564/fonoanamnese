...
export const vocalizationHistorySchema = z
  .object({
...
export const earlyLanguageDevelopmentSchema = z
  .object({
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      data.estimatedProducedWords !== undefined &&
      data.estimatedProducedWords > 3000 &&
      !data.observations
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          'Para estimativas maiores que 3000 palavras, por favor adicione uma observação justificando',
        path: ['observations'],
      })
    }
  })

export const alternativeCommunicationSchema = z
  .object({
...
export const communicationRegressionSchema = z
  .object({
...
export const communicationDevelopmentSchema = z
  .object({
