...
describe('Pregnancy, Birth and Neonatal Schemas', () => {
  describe('pregnancyHistorySchema', () => {
    ...
    it('should require description if infections is sim', () => {
      ...
      expect(result.success).toBe(false);
      const result2 = pregnancyHistorySchema.safeParse({ infections: 'sim', infectionsDescription: 'Zika' });
      expect(result2.success).toBe(true);
    });
    ...
    it('should require description if medications is sim', () => {
      ...
      expect(result.success).toBe(false);
      const result2 = pregnancyHistorySchema.safeParse({ medications: 'sim', medicationsDescription: 'Paracetamol' });
      expect(result2.success).toBe(true);
    });
    ...
    it('should require description if otherSubstances is sim', () => {
      ...
      expect(result.success).toBe(false);
      const result2 = pregnancyHistorySchema.safeParse({ otherSubstances: 'sim', otherSubstancesDescription: 'Drogas ilícitas' });
      expect(result2.success).toBe(true);
    });
    ...
    it('should require description if hospitalizations is sim', () => {
      ...
      expect(result.success).toBe(false);
      const result2 = pregnancyHistorySchema.safeParse({ hospitalizations: 'sim', hospitalizationsDescription: 'Motivo' });
      expect(result2.success).toBe(true);
    });
    ...
    it('should require description if alteredExams is sim', () => {
      ...
      expect(result.success).toBe(false);
      const result2 = pregnancyHistorySchema.safeParse({ alteredExams: 'sim', alteredExamsDescription: 'Ultrasom alterado' });
      expect(result2.success).toBe(true);
    });
  });
  ...
  describe('birthHistorySchema', () => {
    ...
    it('should require description if deliveryType is outro', () => {
      ...
      expect(birthHistorySchema.safeParse({ deliveryType: 'outro', deliveryTypeOther: 'Domiciliar' }).success).toBe(true);
    });
    ...
    it('should require nicuDurationAndReason if nicuAdmission is sim', () => {
      ...
      expect(birthHistorySchema.safeParse({ nicuAdmission: 'sim', nicuDurationAndReason: '3 dias por desconforto respiratório' }).success).toBe(true);
    });
  ...
  describe('neonatalHistorySchema', () => {
    it('should require description if tubeFeeding is sim', () => {
      ...
      expect(neonatalHistorySchema.safeParse({ tubeFeeding: 'sim', tubeTypeAndDuration: 'Orogástrica por 5 dias' }).success).toBe(true);
    });
    ...
    it('should require description if neonatalInfections is sim', () => {
      ...
      expect(neonatalHistorySchema.safeParse({ neonatalInfections: 'sim', neonatalInfectionsDescription: 'Sepse precoce' }).success).toBe(true);
    });

    it('should require description if seizures is sim', () => {
      expect(neonatalHistorySchema.safeParse({ seizures: 'sim' }).success).toBe(false);
      expect(neonatalHistorySchema.safeParse({ seizures: 'sim', seizuresFrequencyAndConduct: '2x, phenobarbital' }).success).toBe(true);
    });
    ...
    it('should allow failed or inconclusive hearing screening without specific schema requirement', () => {
      ...
      expect(neonatalHistorySchema.safeParse({ newbornHearingScreening: 'falhou' }).success).toBe(true);
      expect(neonatalHistorySchema.safeParse({ newbornHearingScreening: 'inconclusivo' }).success).toBe(true);
    });

    it('should allow tongue tie test results', () => {
      expect(neonatalHistorySchema.safeParse({ tongueTieTest: 'alterado', tongueTieTestResult: 'Frenotomia realizada' }).success).toBe(true);
    });

    it('should allow not informed when expected', () => {
      expect(neonatalHistorySchema.safeParse({ jaundice: 'não informado', newbornHearingScreening: 'não informado' }).success).toBe(true);
    });
  });
});
