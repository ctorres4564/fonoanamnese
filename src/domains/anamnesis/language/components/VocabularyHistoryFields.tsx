      ...
      {isOpen && (
        <div className="space-y-6 pt-4">
          <div className="bg-blue-50 text-blue-800 p-4 rounded-md text-sm mb-4">
            Estimativas familiares de vocabulário são dados de anamnese e não substituem avaliação
            formal.
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Vocabulário Compreendido Estimado
              </label>
              <input
                type="number"
                min="0"
                {...register(
                  'languageDevelopment.vocabularyHistory.estimatedComprehendedVocabulary',
                  { valueAsNumber: true },
                )}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Vocabulário Produzido Estimado
              </label>
              <input
                type="number"
                min="0"
                {...register('languageDevelopment.vocabularyHistory.estimatedProducedVocabulary', {
                  valueAsNumber: true,
                })}
                className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              />
          ...
          <div className="space-y-4 border-t pt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                {
                  key: 'restrictedToSpecificInterests',
                  label: 'Vocabulário restrito a interesses específicos?',
                },
              ].map((item) => (
                <div key={item.key}>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
