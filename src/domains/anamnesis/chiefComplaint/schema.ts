...
export const chiefComplaintSchema = z
  .object({
    ...
    evolution: z
      .enum(['melhorando', 'estável', 'piorando', 'oscilante', 'não informado'])
      .optional(),
  ...
  .superRefine((data, ctx) => {
    if (
      ...
      (!data.onsetModeDescription || data.onsetModeDescription.trim() === '')
    ) {
      ctx.addIssue({
    ...
    if (
      data.functionalImpacts?.includes('outro') &&
      (!data.functionalImpactsOther || data.functionalImpactsOther.trim() === '')
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
