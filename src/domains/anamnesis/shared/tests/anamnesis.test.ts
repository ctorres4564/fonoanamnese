...
import { interviewDataSchema, chiefComplaintSchema, createAnamnesisSchema } from '../..'
...
describe('Anamnesis Schemas - InterviewData', () => {
  ...
  it('exige justificativa se informação for limitada', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues.some((i) => i.path.includes('informationLimitationReason'))).toBe(
        true,
      )
    }
  })
