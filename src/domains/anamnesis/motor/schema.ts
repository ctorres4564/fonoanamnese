...
export const motorMilestoneSchema = z.object({
  status: z
    .enum(['adquirido', 'não adquirido', 'em aquisição', 'não informado', 'não se aplica'])
    .optional(),
  acquisitionAgeMonths: z
    .number()
  ...
  acquisitionMode: z
    .enum(['espontânea', 'com estímulo', 'após intervenção', 'não informado'])
    .optional(),
  observations: z.string().optional(),
})

export const motorRegressionSchema = z
  .object({
    ...
    if (data.hasRegression === 'sim') {
      if (!data.lostSkill || data.lostSkill.trim() === '') {
        ctx.addIssue({
          ...
          message: 'Habilidade perdida é obrigatória',
          path: ['lostSkill'],
        })
      }
      if (data.regressionAge === undefined || isNaN(data.regressionAge)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Idade aproximada da regressão é obrigatória',
          path: ['regressionAge'],
        })
      }
    }
...
export const physiotherapyHistorySchema = z
  .object({
...
const motorGeneralSchema = z
  .object({
    ...
    manualPreference: z
      .enum(['direita', 'esquerda', 'alternada', 'ainda não definida', 'não informado'])
      .optional(),
  ...
  .superRefine((data, ctx) => {
    if (
      data.reportedMotorDelay === 'sim' &&
      (!data.motorDelayDescription || data.motorDelayDescription.trim() === '')
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Descrição é obrigatória',
        path: ['motorDelayDescription'],
      })
    }
    if (
      data.usesMobilityDevice === 'sim' &&
      (!data.mobilityDeviceType || data.mobilityDeviceType.trim() === '')
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Tipo de dispositivo é obrigatório',
        path: ['mobilityDeviceType'],
      })
    }
  })
...
export const motorDevelopmentSchema = z.object({
  ...
  general: motorGeneralSchema.optional(),
  regression: motorRegressionSchema.optional(),
  physiotherapy: physiotherapyHistorySchema.optional(),
})
